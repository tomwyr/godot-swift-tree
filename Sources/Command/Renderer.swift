struct NodeTreeRenderer {
    func render(tree: NodeTree) -> String {
        let header = renderHeader()
        let nodeTree = renderNodeTree(scenes: tree.scenes)
        let sceneNodes = tree.scenes.map(renderScene).joinLines(spacing: 2).trimEmptyLines()
